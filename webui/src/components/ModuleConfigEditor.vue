      ...
      <form v-if="editing" class="mt-3 border-top pt-3" @submit.prevent="saveConfig">
        ...
        <template v-if="type !== 'libraryposter' && (type !== 'filemove' || form.backend === 'openlist')">
          ...
          <div class="row g-3">
            ...
            <Field v-if="type === 'alistsync' || type === 'filemove'" label="QPS 限制（0=不限流）" class="col-md-4">
              <input v-model.number="form.qps_limit" type="number" min="0" class="form-control">
              <div class="form-text">对 OpenList API 的请求限流（次/秒），0 表示不限流。</div>
            </Field>
          </div>
        </template>
        ...
        <template v-if="type === 'filemove'">
          <h6 class="mt-4 mb-3">递归移动规则</h6><div class="row g-3">
            ...
            <Switch v-model="form.remove_matched_dirs" label="移动后删除已匹配的子目录" />
        ...
        <template v-else-if="type === 'alistsync'">
          ...
          <h6 class="mt-4 mb-3">并发</h6><div class="row g-3">
            <Field label="同步 worker 数（0=跟随后端）" class="col-md-4"><input v-model.number="form.max_workers" type="number" min="0" class="form-control"></Field>
          </div>
          <div class="form-text">0 表示读取后端 copy_task_threads_num（当前为 5）作为并发数；失败任务由守护协程按退避重试。</div>
...
function normalize(v){const x=clone(v); x.smart_protection ||= {enabled:true,threshold:100,grace_scans:3}; x.configs ||= []; x.pairs ||= []; x.retry ||= {max_attempts:10,backoff:'expo',jitter:.2}; if(props.type==='filemove'){x.backend ??= 'local'; x.regex ??= ''; x.rename_regex ??= ''; x.rename_replacement ??= ''; x.remove_matched_dirs ??= false; x.size ??= null; x.min_size ??= 0; x.max_size ??= 0; x.flatten ??= false; x.overwrite ??= false} if(props.type==='alistsync'||props.type==='filemove'){x.qps_limit ??= 0} if(props.type==='alistsync'){x.max_workers ??= 0} return x}

<template>
  <div>
    ...
    <section class="card mb-4"><div class="card-header d-flex justify-content-between align-items-center"><strong>告警</strong><div class="d-flex gap-2 align-items-center"><label class="small text-muted mb-0"><input type="checkbox" v-model="showAcked" class="form-check-input me-1">显示已确认</label><button v-if="unackedCount" class="btn btn-sm btn-outline-primary" @click="ackAll">一键确认全部 ({{ unackedCount }})</button></div></div><div class="card-body table-responsive"><table class="table"><thead><tr><th>时间</th><th>级别</th><th>来源</th><th>消息</th><th></th></tr></thead><tbody><tr v-for="a in visibleAlerts" :key="a.id"><td>{{ fmt(a.created_at) }}</td><td><span class="badge bg-danger">{{ a.level }}</span></td><td>{{ a.source }}</td><td>{{ a.message }}</td><td><button v-if="!a.acknowledged" class="btn btn-sm btn-outline-secondary" @click="ack(a)">确认</button><span v-else class="text-success">已确认</span></td></tr></tbody></table><div v-if="!visibleAlerts.length" class="text-muted">暂无未确认告警</div></div></section>
...
const unackedCount=computed(()=>alerts.value.filter(a=>!a.acknowledged).length)
const visibleAlerts=computed(()=>showAcked.value?alerts.value:alerts.value.filter(a=>!a.acknowledged))
...
async function ackAll(){await fetch('/api/alerts/ack-all',{method:'POST'});load()}
let timer=null
